...
def init_config(
    ...
    # Build TOML content (avoid backslashes inside f-string for Python 3.10/3.11 compat)
    output_dir_str = str(output_dir).replace("\\", "/")
    auto_resume_str = "true" if auto_resume else "false"
    if api_key_input:
        api_key_line = f'api_key = "{api_key_input}"'
    else:
        api_key_line = '# api_key = "your_key_here"'
